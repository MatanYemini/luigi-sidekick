...
def extract_programming_info(issue_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Extract relevant information for programming tasks from the Jira issue.
    
    Args:
        issue_data: The JSON response from Jira API
        
    Returns:
        A dictionary containing relevant programming information
    """
    ...
    description = extract_text_from_description(description_data)
    ...
    # Extract components
    components = [comp.get("name", "Unknown") for comp in issue_data.get("fields", {}).get("components", [])]
    
    # Extract acceptance criteria if available (often in custom fields)
    acceptance_criteria = ""
    custom_fields = [field for field in issue_data.get("fields", {}) 
                    if field.startswith("customfield_")]
    ...
    for field in custom_fields:
        ...
        if field_value and field_meta and ("acceptance" in field_meta or "criteria" in field_meta):
            if isinstance(field_value, dict) and field_value.get("type") == "doc":
                acceptance_criteria = extract_text_from_description(field_value)
            elif isinstance(field_value, str):
                acceptance_criteria = field_value
    
    # Compile programming task information
    programming_info = {
        "issue_key": issue_key,
        "title": title,
